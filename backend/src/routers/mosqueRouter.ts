...
import { Mosque } from "../db/entities.js";
import { mosqueSchema } from "../schemas/mosqueSchema.js";
  ...
  getMosqueByCoordinates: publicProcedure
    ...
    .query(async ({ input }) => {
        ...
        for (const [key, value] of Object.entries(input.filters)) {
          ...
          if (value) {
            if (key === "name") {
              ...
              Object.keys(mosqueRepo.metadata.propertiesMap).includes(key)
            ) {
      ...
      if (input.coordinates) {
        ...
        query.andWhere(
          "mosque.latitude IS NOT NULL AND mosque.longitude IS NOT NULL",
        );
