...
import { z } from "zod";
import { HeritageSite } from "../db/entities/HeritageSite.entity.js";
import { heritageSchema } from "../schemas/heritageSchema.js";
...
export const heritageRouter = router({
  getPaginated: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (filterName) {
        query.andWhere("heritage.name ILIKE :name", {
          name: `%${filterName}%`,
        });
      }
  ...
  getHeritageByCoordinates: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const query = heritageSiteRepo
        .createQueryBuilder("heritage")
        .where(
          "heritage.latitude IS NOT NULL AND heritage.longitude IS NOT NULL",
        );

      if (input.filters) {
        for (const [key, value] of Object.entries(input.filters)) {
          if (value) {
            query.andWhere(`heritage.${key} ILIKE :${key}`, {
              [key]: `%${value}%`,
            });
          }
        }
