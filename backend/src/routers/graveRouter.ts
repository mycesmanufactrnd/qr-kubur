...
import { Grave } from "../db/entities.js";
import { AppDataSource } from "../datasource.js";
import { z } from "zod";
import { graveSchema } from "../schemas/graveSchema.js";
...
export const graveRouter = router({
  getPaginated: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (filterBlock) {
        query.andWhere("grave.block ILIKE :block", {
          block: `%${filterBlock}%`,
        });
      }
        ...
        for (const [key, value] of Object.entries(input.filters)) {
          ...
          if (value) {
            if (key === "name") {
              ...
              Object.keys(graveRepo.metadata.propertiesMap).includes(key)
            ) {
