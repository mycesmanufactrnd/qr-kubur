...
export default function FamilyTree({
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
        ...
        <p className="text-sm mt-1">
          Click "Add Place" to start building your collection
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center gap-0 select-none w-full pb-8">
      ...
      <div className="relative flex items-start w-full justify-center">
        {activeGroups.length > 1 && (
          <div
            className="absolute top-0 left-[calc(50%_-_0.5px)] right-0 h-0.5 bg-border"
          ...
          {activeGroups.map((type) => {
            ...
            return (
              <div key={type} className="flex flex-col items-center">
                ...
                <div
                  className={`px-4 py-2 rounded-xl border text-xs font-semibold flex items-center gap-1.5 ${cfg.color}`}
                >
                ...
                <div className="relative flex flex-wrap justify-center gap-4 items-start max-w-5xl">
                  {typeItems.map((item) => (
...
function PlaceNode({ item, cfg, detailUrl, onRemove }) {
  const Icon = cfg.icon;
  const borderClass =
    cfg.color.split(" ").find((c) => c.startsWith("border-")) ||
    "border-border";

  return (
    <div
      className={`w-44 rounded-xl border p-3 bg-card shadow-sm hover:shadow-md transition-shadow ${borderClass}`}
    >
