...
function StatusBadge({ status }) {
  ...
  if (!cfg) return <span className="px-3 py-1 text-xs rounded-full bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400">{status}</span>;
  const Icon = cfg.icon;
  return (
    ...
    <div className="min-h-screen pb-10">
      ...
      <div className="max-w-2xl mx-auto px-2 pt-6 space-y-4">
        ...
        <div className="flex flex-col items-center text-center gap-2 pb-2">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-500 flex items-center justify-center mb-1">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-4 space-y-3">
          ...
          <div className="space-y-1.5 pt-1">
            ...
            <Input
              ...
              className="w-full h-11 px-4 rounded-xl border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 text-sm text-slate-800 dark:text-slate-200 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-300 focus:border-emerald-300 transition"
            />
          </div>
          ...
          <Button
            ...
            disabled={searching}
            className="w-full h-12 rounded-2xl bg-gradient-to-r from-emerald-600 to-emerald-500 text-white text-sm font-semibold flex items-center justify-center gap-2 active:opacity-80 transition-all disabled:opacity-50"
          >
              ...
              <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700 overflow-hidden divide-y divide-slate-100 dark:divide-slate-700">
                {request.requestorname && (
                  <div className="flex items-center justify-between px-4 py-3">
                    ...
                    <span className="text-sm font-semibold text-slate-700 dark:text-slate-200">{request.requestorname}</span>
                  </div>
                )}
                {tahfizCenter && (
                  <div className="flex items-center justify-between px-4 py-3">
                    ...
                    <span className="text-sm font-semibold text-slate-700 dark:text-slate-200 text-right max-w-[55%]">{tahfizCenter.name}</span>
                  </div>
                )}
                {request.createdat && (
                  <div className="flex items-center justify-between px-4 py-3">
              ...
              {request.serviceamount != null && (
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center justify-between px-3 py-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-xl">
                    ...
                    <span className="text-sm font-semibold text-slate-900 dark:text-slate-100">{formatRM(request.serviceamount)}</span>
                  </div>
                  {request.platformfeeamount != null && (
                    <div className="flex items-center justify-between px-3 py-2.5 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 rounded-xl">
                      ...
                      <span className="text-sm font-bold text-emerald-700 dark:text-emerald-400">{formatRM(Number(request.serviceamount) + Number(request.platformfeeamount))}</span>
                    </div>
                  )}
                </div>
              )}
