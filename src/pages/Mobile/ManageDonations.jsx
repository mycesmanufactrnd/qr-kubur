...
function TransactionStatusBadge({ status }) {
  const map = {
    ...
    "Transfer Pending": "bg-blue-100 text-blue-700",
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5 pb-32">
        <div className="bg-slate-50 rounded-xl p-3 space-y-2 text-sm">
          <div className="flex justify-between gap-2 items-center">
            <span className="text-slate-500 shrink-0">{translate("Amount")}</span>
          ...
          {donation.referenceno && (
            <div className="flex justify-between gap-2">
              <span className="text-slate-500 shrink-0">{translate("Reference No.")}</span>
              <span className="text-slate-800 text-right break-all font-mono text-xs">{donation.referenceno}</span>
            </div>
          )}
        </div>
        ...
        <div className="space-y-3">
          ...
          {onlineTransactionLoading ? (
            ...
            <p className="text-xs text-slate-400">{translate("No online transaction account found")}</p>
          ) : (
            <div className="bg-slate-50 rounded-xl p-3 space-y-2.5 text-sm">
              <div className="flex justify-between gap-2 items-center">
              ...
              {transactionAccount.photourl && resolveFileUrl(transactionAccount.photourl, "online-transaction") && (
                <div className="pt-1">
                  <p className="text-slate-500 mb-1.5 text-xs">{translate("Photo")}</p>
                  <img
                    src={resolveFileUrl(transactionAccount.photourl, "online-transaction")}
                    alt={translate("Transaction proof")}
                    className="w-full h-40 rounded-xl object-cover border border-slate-100"
                  />
                </div>
              )}
            </div>
