  ...
  en: {
    ...
    pillars: [
      ...
      },
      {
        ...
        icon: "🌙",
        short: "Fasting",
        full: "During the holy month of Ramadan, Muslims fast from dawn until sunset, abstaining from food, drink, and other physical needs. Fasting teaches self-discipline, spiritual reflection, and empathy for those who are less fortunate.",
      },
      {
        name: "Hajj",
        icon: "🕋",
        short: "Pilgrimage",
        full: "The Hajj is the annual Islamic pilgrimage to Mecca, Saudi Arabia, that every able-bodied Muslim who can afford it must undertake at least once in their lifetime. It occurs during the Islamic month of Dhul-Hijjah.",
      },
    ],
    articles: [
      {
...
function PillarCard({ pillar, index, accent = "emerald" }) {
  ...
  const accents = {
    emerald: {
      num: "text-emerald-600 bg-emerald-50 border-emerald-100",
      name: "text-slate-800",
      expand: "bg-emerald-50 text-emerald-600",
    },
    teal: {
      num: "text-teal-600    bg-teal-50    border-teal-100",
      name: "text-slate-800",
      expand: "bg-teal-50    text-teal-600",
    },
  };
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
      ...
      >
        <div
          className={`w-7 h-7 rounded-full border flex items-center justify-center shrink-0 text-[11px] font-bold ${a.num}`}
        >
          {index}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <p className={`text-sm font-bold leading-tight ${a.name}`}>
            {pillar.name}
          </p>
        ...
        <div
          className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 transition-transform duration-300 ${a.expand} ${expanded ? "rotate-180" : ""}`}
        >
