...
export default function Tasbih() {
  ...
  const goToStage = (dir) => {
    setStageIndex(
      (prev) => (prev + dir + GUIDED_STAGES.length) % GUIDED_STAGES.length,
    );
    setGuidedCount(0);
    vibrate(100);
  };
    ...
    <div className="min-h-screen bg-slate-50 pb-10">
      ...
      <div className="flex flex-col items-center text-center gap-2 px-4 pb-4 max-w-md mx-auto">
        <h2 className="text-base font-bold text-slate-800">
          {translate("Digital Tasbih")}
        </h2>
        <p className="text-xs text-slate-400">
          {translate("Count your dhikr")}
        </p>
      </div>
      ...
      <div className="max-w-md mx-auto px-4 space-y-4">
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              >
                ...
                {mode === "free" && (
                  <p className="text-white/80 text-xs font-semibold mt-2 z-10">
                    {translate("Tap to Count")}
                  </p>
                )}
              </motion.button>
              ...
              {mode === "guided" && (
                <p className="text-[11px] font-semibold text-slate-400">
                  {translate("Goal")}:{" "}
                  <span className="text-emerald-600">{targetGoal}</span>
                </p>
              )}
            </div>
        ...
        <div className="flex gap-2.5 items-start px-4 py-3.5 bg-blue-50 border border-blue-100 rounded-2xl">
          ...
          <p className="text-xs text-blue-600 leading-relaxed">
            ...
            {translate(
              "Regular dhikr brings peace to the heart and closeness to Allah.",
            )}
          </p>
        </div>
      </div>
