      ...
      <div className="px-4 pb-10 space-y-4 max-w-2xl mx-auto">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm overflow-hidden border border-slate-100 dark:border-slate-700">
          {tahfizDetails.description && (
            <div className="p-4 border-b border-slate-100 dark:border-slate-700">
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">{tahfizDetails.description}</p>
            </div>
          )}
          ...
          {hasContacts && (
            <div className="p-4 space-y-1">
              ...
              {tahfizDetails.phone && (
                <a href={`tel:${tahfizDetails.phone}`} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 active:bg-slate-100 dark:active:bg-slate-700 transition-colors group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-semibold text-slate-700 dark:text-slate-200 truncate">{tahfizDetails.phone}</p>
                  </div>
                  <ChevronRight className="w-4 h-4 text-slate-300 dark:text-slate-600 group-hover:text-slate-400 dark:group-hover:text-slate-500 shrink-0" />
                </a>
              )}

              {tahfizDetails.email && (
                <a href={`mailto:${tahfizDetails.email}`} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 active:bg-slate-100 dark:active:bg-slate-700 transition-colors group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-semibold text-slate-700 dark:text-slate-200 truncate">{tahfizDetails.email}</p>
                  </div>
                  <ChevronRight className="w-4 h-4 text-slate-300 dark:text-slate-600 group-hover:text-slate-400 dark:group-hover:text-slate-500 shrink-0" />
                </a>
              )}
              ...
              {tahfizDetails.url && (
                <a href={tahfizDetails.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700/50 active:bg-slate-100 dark:active:bg-slate-700 transition-colors group">
                  <div className="w-8 h-8 rounded-full bg-purple-50 dark:bg-purple-900/30 flex items-center justify-center shrink-0">
                    <Globe className="w-3.5 h-3.5 text-purple-500" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 font-medium uppercase tracking-wide">Website</p>
                    <p className="text-sm font-semibold text-slate-700 dark:text-slate-200 flex items-center gap-1">
                      Visit Website <ExternalLink className="w-3 h-3 text-slate-400 dark:text-slate-500" />
                    </p>
                  </div>
                  <ChevronRight className="w-4 h-4 text-slate-300 dark:text-slate-600 group-hover:text-slate-400 dark:group-hover:text-slate-500 shrink-0" />
                </a>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col items-center justify-center gap-1 py-4 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-100 dark:border-amber-800">
              <span className="text-2xl font-bold text-amber-600 dark:text-amber-400">{isRequestLoading ? "—" : pending}</span>
              <span className="text-xs font-medium text-amber-500 dark:text-amber-400">Pending</span>
            </div>
            <div className="flex flex-col items-center justify-center gap-1 py-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-100 dark:border-emerald-800">
              <span className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{isRequestLoading ? "—" : completed}</span>
              <span className="text-xs font-medium text-emerald-500 dark:text-emerald-400">Completed</span>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-4">
            ...
            <div className="space-y-2">
              {tahfizDetails.serviceoffered.map((serviceValue, idx) => {
                ...
                return (
                  <div key={idx} className="flex items-center justify-between px-3 py-2.5 bg-slate-50 dark:bg-slate-700 rounded-xl border border-slate-100 dark:border-slate-600">
                    <div className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 shrink-0" />
                      <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{serviceValue}</span>
                    </div>
                    {servicePrice > 0 ? (
                      <span className="text-xs font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-900/20 px-2.5 py-1 rounded-full">
                        RM {servicePrice}
                      </span>
                    ) : (
                      <span className="text-xs text-slate-400 dark:text-slate-500 italic">Free</span>
                    )}
                  </div>
                );
              })}
        ...
        {tahfizPosts.length > 0 && (
          <div className="space-y-3">
            <div className="flex items-center gap-2 px-1">
              ...
              <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Latest Updates</h2>
            </div>
            <div className="space-y-3">
