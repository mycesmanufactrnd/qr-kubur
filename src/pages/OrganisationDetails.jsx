...
export default function OrganisationDetails() {
  ...
  const deadpersonId = searchParams.get("deadpersonId")
    ? Number(searchParams.get("deadpersonId"))
    : null;
  const graveId = searchParams.get("graveId")
    ? Number(searchParams.get("graveId"))
    : null;
  ...
  const createQuoteRunningNoMutation =
    trpc.runningNo.createQuotationRunningNo.useMutation();
  ...
  const {
    ...
    isLoading,
    isError,
  } = trpc.organisation.getById.useQuery(
    { id: organisationId ?? 0 },
        ...
        .then(async (res) => {
          if (res) {
            ...
            if (
              ...
              formData?.selectedAccount?.bankname
            ) {
              try {
  ...
  const onSubmit = async (formData) => {
    ...
    const payeremail =
      (formData.payeremail || "").trim() || "noreply@gmail.com";
    ...
    const quotationDetails = {
      ...
      servicedescription: isFromGrave
        ? formData.servicedescription || null
        : null,
    };
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          {organisation.canbedonated && (
            <DonationButton
              ...
              state={
                Array.isArray(organisation.states)
                  ? organisation.states[0]
                  : "nearby"
              }
            />
                  ...
                  {isFromDeadPerson || isFromGrave ? (
                      ...
                      {serviceItems.map((serviceItem) => {
                        ...
                        return (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              toggleSelectedService(serviceItem.name)
                            }
                          ...
                          >
                            ...
                            <span
                              className={`text-sm font-bold ${isSelected ? "text-violet-600" : "text-slate-500"}`}
                            >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {organisation.serviceoffered.map((name) => (
                        <span
                          key={name}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold bg-violet-50 border border-violet-100 text-violet-700"
                        >
                          {name}
                          {Number(organisation.serviceprice?.[name] || 0) >
                            0 && (
                      ...
                      {paymentPlatforms.map((platform) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <CreditCard
                              className={`w-4 h-4 ${isActive ? "text-violet-600" : "text-slate-400"}`}
                            />
                            <span
                              className={`text-sm font-semibold ${isActive ? "text-violet-700" : "text-slate-600"}`}
                            >
                              {platform.name}
                            </span>
                          </label>
                      ...
                      {selectedPlatform.fields.map((field) =>
                        field.fieldtype === "image" ? (
                          <img
                            ...
                            src={field.value}
                            alt={field.label}
                            className="max-w-xs rounded-xl border border-slate-100"
                          />
                        ) : (
                          <div
                            key={field.key}
                            className="flex justify-between items-center px-4 py-2.5 bg-slate-50 rounded-xl border border-slate-100"
                          >
