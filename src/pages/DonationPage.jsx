...
export default function DonationPage() {
  ...
  useEffect(() => {
    ...
    const matchedState = STATES_MY.find(s => normalizeState(s) === normalizeState(rawState));
    setSelectedState(rawState === 'nearby' ? 'nearby' : matchedState || 'nearby');
  }, []);
  ...
  const payableAmount = useMemo(() => baseAmount > 0 ? baseAmount + Number(MAINTENANCE_FEE || 0) : 0, [baseAmount]);
  ...
  const { organisations = [] } = useGetOrganisationCoordinates(
    shouldFetchOrganisation ? { latitude: userLocation.lat, longitude: userLocation.lng } : null,
    selectedState === 'nearby' ? userState : selectedState,
    filterName, true
  );

  const { data: tahfizCenters = [] } = useGetTahfizCoordinates(
    shouldFetchTahfiz ? { latitude: userLocation.lat, longitude: userLocation.lng } : null,
    selectedState === 'nearby' ? userState : selectedState,
    filterName
  );

  useEffect(() => {
    if (!urlParamType || !['tahfiz', 'organisation'].includes(urlParamType)) return;
  ...
  const paymentPlatforms = useMemo(() => {
    ...
    paymentConfigs.forEach(config => {
      ...
      if (config.paymentfield) map[code].fields.push({ key: config.paymentfield.key, label: config.paymentfield.label, fieldtype: config.paymentfield.fieldtype, value: config.value });
    });
  ...
  const selectedAccount = useMemo(() => {
    ...
    return { bankname: fields.find(f => f.key === 'bank_name')?.value?.trim() || '', accountno: fields.find(f => f.key === 'account_no')?.value?.trim() || '' };
  }, [selectedPlatform]);

  useEffect(() => {
    if (!paymentPlatforms.length) return;
    if (!paymentPlatforms.some(p => p.code === paymentMethod)) setValue('paymentMethod', paymentPlatforms[0].code);
  }, [paymentPlatforms, paymentMethod, setValue]);

  const handlePaymentConfig = async (formData) => {
    if (!formData) return false;
    setLoadingPayment(true);
    sessionStorage.setItem("donationPending", JSON.stringify({ formData, baseAmount, payableAmount, selectedAccount }));
  ...
  const onSubmit = async (formData) => {
    ...
    if (!isValid) return;
    if (!baseAmount) { showError('Sila lengkapkan maklumat derma'); return; }
    ...
    <div className="min-h-screen bg-slate-50 pb-10">
      ...
      <div className="max-w-2xl mx-auto px-4 space-y-4">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <Section title={translate('Donation Recipient')} icon={Heart} accent="rose">
            <div className="space-y-3">
                ...
                <div className="relative flex-1">
                  ...
                  <Input
                    ...
                    onChange={e => {
                      ...
                      if (recipientType === 'tahfiz') { setValue('selectedRecipient', ''); setValue('paymentMethod', ''); }
                    }}
              ...
              <Select
                ...
                onValueChange={v => {
                  if (recipientType === 'organisation' && organisations.length > 0) setValue('selectedRecipient', v);
                  if (recipientType === 'tahfiz' && tahfizCenters.length > 0) setValue('selectedRecipient', v);
                }}
              >
                <SelectTrigger className="h-10 rounded-xl border-slate-200 text-sm">
                  <SelectValue placeholder={recipientType === 'organisation' ? translate('Select organisation') : translate('Select Tahfiz center')} />
                </SelectTrigger>
                <SelectContent>
                  {recipientList.length > 0 ? (
                    ...
                    <SelectItem value="__empty__" disabled>
                      {recipientType === 'organisation' ? 'Tiada organisasi dijumpai' : 'Tiada pusat tahfiz dijumpai'}
                    </SelectItem>
                  )}
                </SelectContent>
              </Select>
          ...
          <Section title={translate('Total Donations')} accent="amber">
            <div className="space-y-3">
                ...
                {DONATION_AMOUNTS.map(amt => {
                  ...
                  return (
                    <Button
                      ...
                      type="button"
                      onClick={() => {
                        if (isActive) { setValue('amount', ''); }
                        else { setValue('amount', String(amt)); setValue('customAmount', ''); }
                      }}
              ...
              <Input
                ...
                value={customAmount}
                onChange={e => { setValue('customAmount', e.target.value); setValue('amount', ''); }}
              ...
              {baseAmount > 0 && (
                <div className="rounded-xl overflow-hidden border border-slate-100">
                  <div className="flex justify-between items-center px-3 py-2 bg-slate-50 text-sm">
                    ...
                    <span className="font-semibold text-slate-700">RM {baseAmount.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center px-3 py-2 border-t border-slate-100 text-sm">
                    ...
                    <span className="font-semibold text-slate-700">RM {Number(MAINTENANCE_FEE).toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center px-3 py-2.5 bg-emerald-50 border-t border-slate-100">
                    <span className="text-sm font-bold text-emerald-800">Jumlah Keseluruhan</span>
                    <span className="text-base font-bold text-emerald-700">RM {payableAmount.toFixed(2)}</span>
                  </div>
                </div>
              )}
            </div>
          ...
          {paymentPlatforms.length > 0 && (
            <Section title="Kaedah Pembayaran" icon={CreditCard} accent="emerald">
                  ...
                  return (
                    <label
                      ...
                      className={`flex items-center gap-3 p-3.5 rounded-xl border cursor-pointer transition-all ${
                        isSelected ? 'border-emerald-400 bg-emerald-50' : 'border-slate-100 bg-slate-50 hover:bg-slate-100'
                      }`}
                    >
                      ...
                      <span className={`text-sm font-medium ${isSelected ? 'text-emerald-800' : 'text-slate-700'}`}>{p.name}</span>
                    </label>
                  );
                })}
                ...
                {selectedPlatform && (
                  <div className="mt-2 p-3 bg-slate-50 rounded-xl border border-slate-100 space-y-2">
                    {selectedPlatform.fields.map(f => (
                      f.fieldtype === 'image'
                        ? <img key={f.key} src={f.value} alt={f.label} className="max-w-[180px] rounded-lg border" />
                        : <p key={f.key} className="text-xs text-slate-600"><span className="font-semibold text-slate-800">{f.label}:</span> {f.value}</p>
                    ))}
                  </div>
                )}
              </div>
          ...
          <Section title={translate('Donor Information (Optional)')} icon={User} accent="blue">
            <div className="space-y-3">
              ...
              <div className="flex gap-2 p-3 bg-blue-50 border border-blue-100 rounded-xl text-xs text-blue-600 leading-relaxed">
                ...
                <span>Resit pembayaran akan dihantar ke emel yang diberikan jika ada.</span>
              </div>
            </div>
          </Section>
          ...
          <button
            type="submit"
            className="w-full h-12 rounded-2xl bg-gradient-to-r from-emerald-600 to-emerald-500 text-white font-semibold text-sm shadow-lg shadow-emerald-200 active:opacity-80 transition-all"
          >
