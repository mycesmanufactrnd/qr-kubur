...
export default function WaqfDetail() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useGetWaqfProjectById(projectId);
      ...
      <div className="relative h-64 md:h-96 overflow-hidden">
        {project.photourl ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-emerald-600 via-teal-500 to-cyan-600 flex items-center justify-center">
            <div className="text-white/10 text-[200px] font-arabic leading-none select-none">
              وقف
            </div>
          </div>
        )}
        ...
        <button
          onClick={() =>
            shareLink({
              title: project.waqfname,
              text: `Visit ${project.waqfname}`,
        ...
        <div className="absolute bottom-0 left-0 right-0 px-5 pb-5 md:px-8 md:pb-8">
          <div className="max-w-6xl mx-auto">
            <div className="flex flex-wrap items-center gap-2 mb-2">
              <span
                className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${CATEGORY_COLORS[project.category] || "bg-slate-100 text-slate-700 border-slate-200"}`}
              >
                {project.category}
              </span>
              <span
                ...
                <div className="rounded-xl border border-slate-100 overflow-hidden divide-y divide-slate-100">
                  <div className="flex items-center justify-between px-4 py-3 bg-emerald-50">
                    ...
                    <span className="text-sm font-bold text-emerald-700">
                      {formatRM(project.totalrequired)}
                    </span>
                  </div>
