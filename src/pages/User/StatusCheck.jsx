...
import {
  Search, BookOpen, Briefcase, Clock, CheckCircle, XCircle,
  MapPin, User, Calendar, Building2, Phone, Mail, Heart,
} from "lucide-react";
...
const TAHLIL_STATUS = {
  ...
  [TahlilStatus.ACCEPTED]:  { label: "Accepted",  Icon: CheckCircle,   bg: "bg-blue-50 dark:bg-blue-900/20",     border: "border-blue-200 dark:border-blue-800",     text: "text-blue-700 dark:text-blue-400",     iconColor: "text-blue-500 dark:text-blue-400" },
...
function InfoRow({ Icon, label, value, breakAll }) {
  ...
  return (
    <div className="flex items-center justify-between px-4 py-3">
      ...
      <span className={`text-sm font-semibold text-slate-700 dark:text-slate-200 text-right max-w-[55%] ${breakAll ? "break-all" : ""}`}>{value}</span>
    </div>
  );
}
...
export default function StatusCheck() {
  ...
  useEffect(() => {
    ...
    if (!tahlilData) { showError(translate("Request not found")); return; }
    setTahlilRequest({
  ...
  const handleTahlilSearch = () => {
    if (!tahlilRef.trim()) { showError(translate("Please enter Reference ID")); return; }
    setTahlilSearching(true);
  ...
  useEffect(() => {
    ...
    if (!quotation) { showError(translate("Service not found")); return; }
    setServiceDialogOpen(true);
  }, [quotation, quotationLoading, serviceKey]);

  const handleServiceSearch = () => {
    if (!serviceRef.trim()) { showError(translate("Please enter Reference No.")); return; }
    setServiceSearching(true);
  ...
  return (
    <div className="min-h-screen pb-10 dark:bg-slate-900">
          ...
          <>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-4 space-y-3">
              ...
              >
                ...
                {tahlilSearching ? translate("Searching...") : translate("Search Status")}
              </Button>
            </div>
            ...
            <div className="flex gap-2.5 items-start px-4 py-3.5 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-2xl">
              ...
              <p className="text-xs text-blue-600 dark:text-blue-400 leading-relaxed">
                ...
                {translate("Your reference number can be found in the payment receipt sent to your email.")}
              </p>
            </div>
          </>
        )}
        ...
        {activeTab === "service" && (
          <>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 p-4 space-y-3">
              ...
              >
                <Search className="w-4 h-4" />
                {serviceSearching ? translate("Searching...") : translate("Search Status")}
              </Button>
            </div>
      ...
      {/* ── Tahlil dialog ── */}
      <Dialog open={tahlilDialogOpen} onOpenChange={(open) => { if (!open) handleTahlilClose(); }}>
        <DialogContent className="max-w-lg w-[calc(100%-2rem)] max-h-[80vh] overflow-y-auto rounded-2xl p-0 border-0 shadow-2xl bg-white dark:bg-slate-900">
          <div className="px-5 pt-5 pb-4 border-b border-slate-100 dark:border-slate-700 text-center">
            ...
            <div className="flex flex-col items-center gap-2.5">
              ...
              <StatusBadge status={tahlilRequest.status} configMap={TAHLIL_STATUS} />
            </div>
          </div>
          ...
          <div className="px-5 py-4 space-y-5">
            ...
            <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700 overflow-hidden divide-y divide-slate-100 dark:divide-slate-700">
              <InfoRow Icon={User} label={translate("Requester")} value={tahlilRequest.requestorname} />
              {tahfizCenter && <InfoRow Icon={MapPin} label={translate("Tahfiz Center")} value={tahfizCenter.name} />}
              {tahlilRequest.createdat && (
                <InfoRow
                  Icon={Calendar}
                  label={translate("Date")}
                  value={new Date(tahlilRequest.createdat).toLocaleDateString("ms-MY", { day: "numeric", month: "long", year: "numeric" })}
                />
              )}
            </div>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5">
                  {tahlilRequest.deceasednames.map((name, i) => (
                    <div key={i} className="flex items-center gap-3 px-3 py-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-xl">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {tahlilRequest.selectedservices.map((type, i) => (
                    <span key={i} className="px-3 py-1.5 rounded-full text-xs font-semibold bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400">
                      {type}
                    </span>
                  ))}
                </div>
            ...
            {tahlilRequest.serviceamount != null && (
              <div className="space-y-1.5">
                ...
                <div className="flex items-center justify-between px-3 py-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-xl">
                  <span className="text-sm text-slate-500 dark:text-slate-400">{translate("Service Amount")}</span>
                  <span className="text-sm font-semibold text-slate-900 dark:text-slate-100">{formatRM(tahlilRequest.serviceamount)}</span>
                </div>
                {tahlilRequest.platformfeeamount != null && (
                  <div className="flex items-center justify-between px-3 py-2.5 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 rounded-xl">
                    <span className="text-sm font-semibold text-emerald-700 dark:text-emerald-400">{translate("Total Paid")}</span>
                    <span className="text-sm font-bold text-emerald-700 dark:text-emerald-400">{formatRM(Number(tahlilRequest.serviceamount) + Number(tahlilRequest.platformfeeamount))}</span>
                  </div>
                )}
              </div>
            )}
            ...
            {tahlilRequest.photourls?.length > 0 && (
              <div className="space-y-2">
                <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-400 px-1">{translate("Tahlil Photos")}</p>
                <div className="grid grid-cols-2 gap-2">
                  {tahlilRequest.photourls.map((url, idx) => (
                    <img key={`${url}-${idx}`} src={resolveFileUrl(url, "bucket-tahfiz-tahlil")} alt={`Tahlil ${idx + 1}`} className="h-24 w-full rounded-lg object-cover border border-slate-100 dark:border-slate-700" />
                  ))}
                </div>
              </div>
            )}

            <Button onClick={handleTahlilClose} variant="outline" className="w-full h-11 rounded-xl dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400">
              {translate("Close")}
            </Button>
          </div>
        </DialogContent>
      ...
      {/* ── Service dialog ── */}
      <Dialog open={serviceDialogOpen} onOpenChange={(open) => { if (!open) handleServiceClose(); }}>
        <DialogContent className="max-w-lg w-[calc(100%-2rem)] max-h-[80vh] overflow-y-auto rounded-2xl p-0 border-0 shadow-2xl bg-white dark:bg-slate-900">
          <div className="px-5 pt-5 pb-4 border-b border-slate-100 dark:border-slate-700 text-center">
            <DialogTitle className="text-[11px] font-semibold uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-3">
            ...
            {quotation && (
              <div className="flex flex-col items-center gap-2.5">
                ...
                <StatusBadge status={quotation.status} configMap={SERVICE_STATUS} />
              </div>
            )}
          </div>
          ...
          {quotation && (
            <div className="px-5 py-4 space-y-5">
              <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700 overflow-hidden divide-y divide-slate-100 dark:divide-slate-700">
                <InfoRow Icon={User} label={translate("Requester")} value={quotation.payername} />
                <InfoRow Icon={Mail} label={translate("Email")} value={quotation.payeremail} breakAll />
                <InfoRow Icon={Phone} label={translate("Phone No")} value={quotation.payerphone} />
                <InfoRow Icon={Building2} label={translate("Organisation")} value={quotation.organisation?.name} />
                <InfoRow Icon={Heart} label={translate("Deceased")} value={quotation.deadperson?.name} />
                {quotation.createdat && (
                  <InfoRow
                    Icon={Calendar}
                    label={translate("Date")}
                    value={new Date(quotation.createdat).toLocaleDateString("ms-MY", { day: "numeric", month: "long", year: "numeric" })}
                  />
                )}
              </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1.5">
                    {quotation.selectedservices.map((s, i) => (
                      <div key={i} className="flex items-center justify-between px-3 py-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700 rounded-xl">
              ...
              {quotation.photourl && (
                <div className="space-y-2">
                  <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-400 px-1">{translate("Service Photos")}</p>
                  <img src={resolveFileUrl(quotation.photourl, "bucket-organisation-services-proof")} alt="Service completion" className="h-48 w-full rounded-xl object-cover border border-slate-100 dark:border-slate-700" />
                </div>
              )}

              <Button onClick={handleServiceClose} variant="outline" className="w-full h-11 rounded-xl dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400">
                {translate("Close")}
              </Button>
            </div>
          )}
