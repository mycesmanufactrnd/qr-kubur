...
export default function ManageWaqfProject() {
  ...
  const { createWaqfProject, updateWaqfProject, deleteWaqfProject } =
    useWaqfProjectMutations();
  ...
  const handleSubmit = async (data) => {
    if (editingProject) {
      updateWaqfProject
        .mutateAsync({ id: editingProject.id, data })
        .then((res) => {
          if (res) {
            setShowDialog(false);
          }
      ...
      createWaqfProject.mutateAsync(data).then((res) => {
        if (res) {
          setShowDialog(false);
        }
      });
      ...
      <div className="space-y-6">
        <Breadcrumb
          items={[
            ...
            {
              label: translate("Manage Waqf Project"),
              ...
              >
                <CardContent className="p-4 sm:p-4">
                  <div className="flex items-start justify-between">
                    <div>
                      <p
                        className={`${stat.textColor} text-xs sm:text-sm mb-1`}
                      >
                        {stat.label}
                      </p>
