...
import { BookOpen, CheckCircle, XCircle, Clock, Eye, Video, Trash2, Upload, Search, X } from 'lucide-react';
...
export default function ManageTahlilRequests() {
  ...
  }, [urlStatus, urlReference, urlService, urlTahfiz, isSuperAdmin, lockedTahfizValue]);
  ...
  const handleStatusChange = async (newStatus) => {
    ...
    const payload = { status: newStatus, suggesteddate: '', photourls: tahlilPhotoUrls };
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="all">{translate('All Tahfiz Centers')}</SelectItem>
                      ...
                      {request.status === TahlilStatus.ACCEPTED && (
                        <>
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => openUploadDialogForRequests([request.id])}
      ...
      >
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto dark:bg-gray-800 dark:border-gray-700">
          <DialogHeader>
                    ...
                    <div className="space-y-2">
                      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3">
                        <Input
                          ...
                          type="file"
                          accept="image/*"
                          onChange={(e) => handleDialogFileChange(e.target.files?.[0])}
                          disabled={
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                type="file"
                accept="image/*"
                onChange={(e) => handleUploadDialogFileChange(e.target.files?.[0])}
                disabled={tahlilPhotoUploading}
              />
              {uploadDialogPreview && (
                <div className="space-y-1">
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          {translate('Reference No.')}: {request.referenceno || '-'}
                        </p>
                        {photoUrl ? (
                          <img
