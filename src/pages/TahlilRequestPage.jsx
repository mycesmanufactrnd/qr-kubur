...
import { Building2, CreditCard, Plus, Trash2, ChevronDown, Info, CheckCircle2 } from 'lucide-react';
...
export default function TahlilRequestPage() {
  ...
  const { data: nearbyTahfiz = [], isLoading: isTahfizLoading } = useGetTahfizCoordinates(
  ...
  const toggleServiceType = (value) => {
    ...
    setValue('selectedservices', isSelected ? current.filter(v => v !== value) : [...current, value]);
    if (value === CUSTOM_SERVICE_KEY && isSelected) setValue('customservice', '');
  };

  const handleAddDeceased = () => setValue('deceasednames', [...watch('deceasednames'), '']);
    ...
    <div className="min-h-screen bg-slate-50 pb-10">
      ...
      <div className="max-w-2xl mx-auto px-4 space-y-4">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          {selectedTahfiz && (
            <Section title="Jenis Perkhidmatan" icon={CheckCircle2} accent="violet">
              <div className="space-y-2">
                {selectableTahfizServices.map(serviceValue => {
                  const isSelected = (selectedservices || []).includes(serviceValue);
                  const serviceLabel = serviceValue === CUSTOM_SERVICE_KEY ? CUSTOM_SERVICE_LABEL : serviceValue;
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`flex-1 text-sm font-medium ${isSelected ? 'text-violet-800' : 'text-slate-700'}`}>
                        {serviceLabel}
                      </span>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
              <p className="text-[11px] font-semibold uppercase tracking-widest text-rose-500">Maklumat Arwah</p>
              <button
                type="button"
          ...
          {selectedTahfiz && paymentPlatforms.length > 0 && (
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-3 border-b border-slate-100">
                ...
                <p className="text-[11px] font-semibold uppercase tracking-widest text-emerald-600">Kaedah Pembayaran</p>
              </div>
              <div className="p-4 space-y-2">
              ...
              <div className="mx-4 mb-4 rounded-xl border border-slate-100 overflow-hidden">
                {hasService && (
                  <div className="p-3 bg-slate-50 border-b border-slate-100">
                    <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500 mb-2">Servis Dipilih</p>
                    <div className="space-y-1">
                      {validServiceTypes.map(type =>
                        hasTahfizServicePrice(type) && (
                          <div key={type} className="flex justify-between text-sm">
                            <span className="text-slate-600">{type}</span>
                            <span className="font-semibold text-slate-800">RM {selectedTahfizServicePrice[type]}</span>
                          </div>
                        )
                      )}
                    </div>
                ...
                {hasDonation && (
                  <div className="flex justify-between items-center p-3 bg-amber-50 border-b border-slate-100 text-sm">
                    ...
                    <span className="font-semibold text-amber-700">RM {donationAmount.toFixed(2)}</span>
                  </div>
                )}
                ...
                {platformFee > 0 && (
                  <div className="flex justify-between items-center p-3 border-b border-slate-100 text-sm">
                    ...
                    <span className="text-slate-600">RM {platformFee.toFixed(2)}</span>
                  </div>
                )}
                ...
                <div className="flex justify-between items-center p-3 bg-emerald-50">
                  ...
                  <span className="text-base font-bold text-emerald-700">RM {finalAmountWithFee.toFixed(2)}</span>
                </div>
              </div>
            </div>
          )}
